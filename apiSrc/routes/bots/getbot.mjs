...
export const route = {
	...
	handler: async (request, reply) => {
        const bot = await db`SELECT addedon, bots.username, avatar, public, bots.ownerid AS ownerid, owners.username AS ownername FROM bots JOIN owners ON bots.ownerid = owners.ownerid WHERE botid = ${request.params.id}`.catch(() => {})
