...
export default class RequestHandler extends EventEmitter {
	constructor(options={}) {
		...
		this.userAgent = `Discord-OAuth2 (https://github.com/reboxer/discord-oauth2, 2.10.0)`;
		this.ratelimits = {};
		this.latencyRef = {
			...
			const actualCall = (cb) => {
				...
				req.once("response", (resp) => {
					...
					_respStream.on("data", (str) => {
						...
						if (method !== "GET" && (resp.headers["x-ratelimit-remaining"] === undefined || resp.headers["x-ratelimit-limit"] === undefined) && this.ratelimits[url].limit !== 1) {
							this.emit("debug", `Missing ratelimit headers for SequentialBucket(${this.ratelimits[url].remaining}/${this.ratelimits[url].limit}) with non-default limit\n`
                                + `${resp.statusCode} ${resp.headers["content-type"]}: ${method} ${url} | ${resp.headers["cf-ray"]}\n`
                                + "content-type = " + "\n"
						...
						if (retryAfter >= 0) {
							if (resp.headers["x-ratelimit-global"]) {
								...
								this.ratelimits[url].reset = (retryAfter || 1) + now;
							}
						}
