<template>
    <div class="grid md:grid-cols-3 sm:grid-cols-1 gap-4">
        <router-link :to="'/bots/' + bot.botid+'/'" v-for="bot in bots" v-bind:key="bot.botid">
            <UCard >
                <UUser :name="bot.username" :description="bot.shortdesc" orientation="horizontal" :avatar="{src: `https://cdn.discordapp.com/avatars/${bot.botid}/${bot.avatar}.webp?size=512${bot.avatar?.startsWith('a_')?'&animated=true':''}`, chip: {color: bot.lat ? 'primary': 'error'}, ui:{image:bot.nsfw?'blur':''}}" size="3xl">
